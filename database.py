...
def addClient(telegram_id, samoware_login, samovar_session):
    db.execute("INSERT INTO clients VALUES(?, ?, ?)",(telegram_id, samoware_login, samovar_session))
    db.commit()

def setSession(telegram_id, samovar_session):
    db.execute("UPDATE clients SET samoware_session=? WHERE telegram_id=?",(samovar_session, telegram_id))
    db.commit()
