...
async def activate(telegram_id, samovar_login, samovar_password):
    ...
    if(samovar_session == None):
        ...
        return
    database.addClient(telegram_id, samovar_login, samovar_session)
...
def main():
    ...
    for client in database.loadAllClients():
        ...
        session = samoware_client.loginWithSession(client[1], client[2])
        loop.create_task(client_handler(client[0], session))
        database.setSession(client[0], session)
